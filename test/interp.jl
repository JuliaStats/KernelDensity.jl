...
k = kde((X,Y,Z))
@test pdf(k, k.x, k.y, k.z) ≈ k.density
...
k = kde(([0.0],[0.0],[0.0]), bandwidth=(1.0, 1.0, 1.0))
@test pdf(k, k.x, k.y, k.z) ≈ k.density
@test pdf(k, -10.0, 0.0, 0.0) ≤ pdf(MvNormal(3, 1.0), [-10.0, 0.0, 0.0])
@test pdf(k, +10.0, 0.0, 0.0) ≤ pdf(MvNormal(3, 1.0), [+10.0, 0.0, 0.0])
@test pdf(k, 0.0, -10.0, 0.0) ≤ pdf(MvNormal(3, 1.0), [0.0, -10.0, 0.0])
@test pdf(k, 0.0, +10.0, 0.0) ≤ pdf(MvNormal(3, 1.0), [0.0, +10.0, 0.0])
@test pdf(k, 0.0, 0.0, +10.0) ≤ pdf(MvNormal(3, 1.0), [0.0, 0.0, +10.0])
@test pdf(k, 0.0, 0.0, -10.0) ≤ pdf(MvNormal(3, 1.0), [0.0, 0.0, -10.0])
